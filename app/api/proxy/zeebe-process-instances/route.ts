...
const ZEEBE_API = {
  ...
  processInstancesEndpoint: '/v1/process-instances/search'
};

export async function POST(request: NextRequest) {
  console.log('🚀 Zeebe Process Instances proxy called');
  
  try {
    const body = await request.json();
    const url = `${ZEEBE_API.baseUrl}${ZEEBE_API.processInstancesEndpoint}`;
    
    console.log('📤 Proxying process instances request to:', url);
    ...
    return NextResponse.json(
      { 
        error: 'Failed to proxy process instances request to Zeebe API',
        details: error instanceof Error ? error.message : 'Unknown error',
        zeebeUrl: `${ZEEBE_API.baseUrl}${ZEEBE_API.processInstancesEndpoint}`
      },
      { 
        status: 500,
        headers: {
